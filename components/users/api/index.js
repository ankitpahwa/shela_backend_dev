...
const searchEmployeeHome = async (request, h) => {
    const log = logger.start('users:api:searchEmployeeHome')

    try {
        // request.payload.userId = request.userInfo._id
        const message = await service.searchEmployeeHome(request.payload)
        log.end()
        return response.success(h, message)
    } catch (err) {
        log.error(err)
