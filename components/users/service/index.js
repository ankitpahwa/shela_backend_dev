...
const signUp = async (params) => {
    ...
    if (params.userRole == "employer") {
        offline.queue('user', 'sign-up-employer', {
            id: user.id
        }, {})
    } else {
...
const login = async (params) => {
    ...
    const data = await db.users.findOneAndUpdate({ email: params.email, isDeleted:false }, dataToUpdate).lean();
    log.end()
...
const employeeProfileStep1 = async (params) => {
    ...
    if(params.currentPositionId !="" && !ObjectId.isValid(params.currentPositionId)){
        let currentPositionCriteria = {
            $or:[
                {name:params.currentPositionId},
            ]
        };

        let currentPositionId = await db.currentPosition.findOne(currentPositionCriteria,{ _id: 1 }).lean();
        if(currentPositionId){
            params.currentPositionId = currentPositionId._id;
        }
...
const employeeProfileStep2 = async (params) => {
    ...
    const qualification = {
        ...
        $set: { profileStepCompleted: profileStepCompleted,isProfileCompletedStep2: true }
    }
...
const employeeProfileStep3 = async (params) => {
    ...
    const employmentInfo = {
        $set: { profileStepCompleted: profileStepCompleted,isProfileCompletedStep3: true },
...
const employeeProfileStep4 = async (params) => {
    ...
    const data = await db.users.findOneAndUpdate({ _id: params.userId }, { $set: { "document.cvId": params.cvId, "document.pvrId": params.pvrId, profileStepCompleted: 4, bio: params.bio,isProfileCompletedStep4: true } }, { new: true })
...
const sendConnectionRequest = async (params) => {
    ...
    const obj = {
        $addToSet: { associateRequests: { toContactId: params.toContactId, from: params.userId, requestStatus: 1 } }
    }

    await db.users.update({ _id: params.toContactId }, {
        '$pull': {
            associateRequests:
            {
                "requestStatus": 1,
    ...
    await db.users.update({ _id: params.userId }, {
        '$pull': {
            sentRequests:
            {
                "requestStatus": 1,
    ...
    const data1 = await db.users.findOneAndUpdate({ _id: params.userId }, { $addToSet: { sentRequests: { toContactId: params.toContactId, requestStatus: 1 } } }, { new: true });
    log.end()
...
const viewSavedJobs = async (params) => {
    ...
    //await data.forEach(async (obj) =>
    {
        ...
        const y = await db.job.find({ "_id": { $in: params.userInfo.savedJobs } }, { companyId:1 }, {  })
        const username = await db.users.findOne({ "_id": y[0]['companyId'] }, { fullName:1,companyLogo:1 }, {  })
        ...
        obj1 = {
            ...
            companyNames: username.fullName,
            companyLogo: username.companyLogo,
...
const companyList = async (params) => {
    ...
    if (check1.followedCompanies != "") {
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            if(data[i] !== undefined){
                console.log('inside the if-durgesh')
                for (var j = 0; j < check1.followedCompanies.length; j++) {
...
const associateList = async (params) => {
    ...
    const check1 = await db.users.findOne({ _id: params.userId }, { "associateConnected": 1,"sentRequests":1,"associateRequests":1  }).populate({ path: "associateConnected.contactId", model: "users", select: "currentPositionId address fullName profilePicId", populate: { path: "currentPositionId", select: "name" } }).lean();
    ...
    // const data = await db.users.find({ userRole: "employee", profileStepCompleted: 4 }, { _id: 1, currentPositionId: 1, address: 1, fullName: 1, profilePicId: 1 }).lean().skip(params.skip).limit(params.limit);

    //for checking associate connection status
    if (data && data.length > 0) {
        await data.forEach(element => {
            element.currentPositionName = element.currentPositionId ? element.currentPositionId.name: element.currentPositionId;
            delete element.currentPositionId;
            let index;
            if (check1 && check1.associateConnected && check1.associateConnected.length > 0) {
                index = check1.associateConnected.findIndex(item => item.contactId._id.toString() == element._id.toString() && item.requestStatus == 2);
            }
            if (index > -1) {
            ...
            // element.isConnected= check1 && check1.associateConnected && check1.associateConnected.length? (check1.associateConnected.findIndex(i=>i.contactId._id.toString() == element._id.toString() && i.requestStatus == 2)>-1)?true:false: false;
            
            element.isSendRequest= check1 && check1.sentRequests && check1.sentRequests.length? (check1.sentRequests.findIndex(i=>i.toContactId.toString() == element._id.toString() && i.requestStatus == 1)>-1)?true:false: false;

            element.isRecivedRequest= check1 && check1.associateRequests && check1.associateRequests.length? (check1.associateRequests.findIndex(i=>i.from.toString() == element._id.toString() && i.requestStatus == 1)>-1)?true:false: false;

            /* status
                0 for no requset, 1 for send request, 2 for connected, 3 for recive request
            */
            switch(true){
                case(element.isConnected): element.status=2; break;
                case(element.isSendRequest): element.status=1; break;
                case(element.isRecivedRequest): element.status=3; break;
                default:element.status=0;
            }
...
const employerJobStats = async (params) => {
    ...
    obj.currentJobPosted1 = await db.job.find({ companyId: params.userId, status: 1 })
    obj.currentJobPosted = await db.job.find({ companyId: params.userId, status: 1 }).count()
    console.log("obj.currentJobPosted1",obj.currentJobPosted1);
    console.log("obj.currentJobPosted",obj.currentJobPosted);
    return obj;
...
const searchEmployeeHome = async (params) => {
    const log = logger.start('users:service:searchEmployeeHome')
    ...
    // data = await db.users.find(obj, { employmentInfo: 1, _id: 1, fullName: 1, totalExperience: 1, profilePicId: 1, isWorking: 1, "document.pvrId": 1 },{ lean: true }).sort({_id: -1})
    data = await db.users.find(obj, { },{ lean: true }).sort({_id: -1})
...
const viewInvitedJobs = async (params) => {
    ...
    //await data.forEach(async (obj) =>
    {
        if(data.jobInvites[counter].jobId != null)  {
...
const selfProfile = async (params) => {
    const data = await db.users.findOne({ _id: params.userId }, {
        ...
        followedCompanies: 1,
        profileStepCompleted: 1,
        isProfileCompletedStep1: 1,
        isProfileCompletedStep2: 1,
        isProfileCompletedStep3: 1,
        isProfileCompletedStep4: 1,
        // totalCredits: 1,
        // availableCredits: 1,
        // usedCredits: 1,
    })
        .populate({ path: "skills", model: "skills", select: "name" })
        .populate({ path: "currentPositionId", select: "name" })
...
const fetchReviews = async (params) => {
    const log = logger.start('users:service:fetchReviews')

    const check1 = await db.users.findOne({ _id: params.employerId }, { review: 1 }, { lean: true })
    if (check1.review != "") {
        let counter = 0
        let res = []
        let data1 = {}
...
const socialLoginSignUp = async (params) => {
    ...
    if (params.appleLoginId && params.appleLoginId != "")
        dataToSave.appleLoginId = params.appleLoginId
    ...
    if (user) {
        params.userId = await db.users.findOneAndUpdate({ email: params.email, userRole: params.userRole,isDeleted:false }, dataToSave, { new: true })
    } else {
    ...
    const data = await db.users.findOneAndUpdate({ _id: params.userId._id,isDeleted:false }, { deviceDetails: obj }, { new: true })
    data.token = obj.accessToken
    return data

}
const appleLoginSignUp = async (params) => {
    ...
    const user = await db.users.findOne({ appleLoginId: params.appleLoginId, isDeleted: false })

    if (user && user.userRole.toString() != params.userRole.toString()) {
        throw new Error("Apple LoginId already exists")
    }
...
console.log("DATA TO SAVE=======>>>>>",dataToSave,user,"====");
    if (user) {
        params.userId = await db.users.findOneAndUpdate({ _id: user._id, userRole: params.userRole }, dataToSave, { new: true })
    } else {
        dataToSave.email = params.email
        dataToSave.isEmailVerified = true
        dataToSave.userRole = params.userRole
    ...
    if(accessData != '' && accessData != null && accessData != undefined){
        ...
        if(availableCredits < deductCredits){
            ...
            if(params.view_type == "message" ){
                ...
                offline.queue('user', 'interviewschedule', {
                    id: params.empId,
                    message:"An employer has recently viewed your Personal Video Resume. Log into your Shela Jobs app to check for any new messages."
                }, {})
            } 
...
const EmployersContactLeads = async (params) => {
    // console.log('INNNNNNNNNNNNNNNNNNNNNNNNNN',params)
        ...
        const obj = {
            name:params.name || params.Empname || params.fullName,
...
exports.searchEmployeeHome = searchEmployeeHome
