...
const job = await db.jobType.findOne({ _id: data[0].jobType }, { name: 1 })
    ...
    if (data.length && data[0].address) {
        data[0].companyFullAddress = data[0].address.fullAddress ? data[0].address.fullAddress : data[0].companyFullAddress;
        data[0].companyCity = data[0].address.city ? data[0].address.city : data[0].companyFullAddress;
    }
...
const searchByJobType = async (params) => {
    ...
    if (params.jobType == "saved") {
        const savedJobs = await db.job.find({ _id: { $in: params.userInfo.savedJobs }, jobTitle: new RegExp(params.search,"i") }, { maxSalary: 1, companyId: 1, minSalary: 1, jobTitle: 1,  jobPostedDate: 1, skillsRequired: 1 }).populate({path:"companyId", select: "fullName companyLogo address "}).lean();

        for (const element of savedJobs) {
            element.jobTitle = element.jobTitle
            element.companyFullAddress = element.companyId.address
        ...
        let newinvited = [];
        params.userInfo.jobInvites.forEach((item,index) => {
          newinvited[index]=item.jobId;
        });
        // console.log('newinvited>>>>>',newinvited)
        const invitedCandidate = await db.job.find({ _id: { $in: newinvited }, jobTitle: new RegExp(params.search,"i") }, { maxSalary: 1, companyId: 1, minSalary: 1, jobTitle: 1,  jobPostedDate: 1, skillsRequired: 1 }).populate({path:"companyId", select: "fullName companyLogo address "}).lean();

        for (const element of invitedCandidate) {
            element.companyFullAddress = element.companyId.address
            element.companyLogo = element.companyId.companyLogo
            element.companyNames = element.companyId.fullName
            let skillsRequired = element.skillsRequired;
        ...
        //    let invitedCandidate = await db.job.findOne({ _id: element.jobId, jobTitle: new RegExp(params.search,"i") }, { maxSalary: 1, companyId: 1, minSalary: 1,selectcurrencyid:1, jobTitle: 1, jobPostedDate: 1 }).populate({path:"companyId", select: "fullName companyLogo address "});
        //         invitedCandidates.push(invitedCandidate);
        // });
        
        // await invitedCandidates.forEach(async element => {
    ...
    // console.log('params.userInfo.appliedJobs',params.userInfo.appliedJobs)
        let newapplied = [];
        params.userInfo.appliedJobs.forEach((item,index) => {
          newapplied[index]=item.jobId;
        });
        const appliedJob = await db.job.find({ _id: { $in: newapplied }, jobTitle: new RegExp(params.search,"i") }, { maxSalary: 1, companyId: 1, minSalary: 1, jobTitle: 1,  jobPostedDate: 1, skillsRequired: 1 }).populate({path:"companyId", select: "fullName companyLogo address "}).lean();
        ...
        //     }
        // });
        await appliedJob.forEach(async element => {
            // element.companyCity = element.companyId.address.city
            element.companyFullAddress = element.companyId.address  
            element.companyLogo = element.companyId.companyLogo
            element.companyNames = element.companyId.fullName
        ...
        // console.log('params.userInfo.appliedJobs',params.userInfo.appliedJobs)
        let newinterviewingRequests = [];
        params.userInfo.appliedJobs.forEach((item,index) => {
          newinterviewingRequests[index]=item.jobId;
        });
        console.log('newinterviewingRequests',newinterviewingRequests)
        const interviewingRequests = await db.job.find({ _id: { $in: newinterviewingRequests }, jobTitle: new RegExp(params.search,"i") }, { maxSalary: 1, companyId: 1, minSalary: 1, jobTitle: 1,  jobPostedDate: 1, skillsRequired: 1 }).populate({path:"companyId", select: "fullName companyLogo address "}).lean();
        ...
        await interviewingRequests.forEach(async element => {
            element.companyFullAddress = element.companyId.address  
            element.companyLogo = element.companyId.companyLogo
            element.companyNames = element.companyId.fullName
            let skillsRequired = element.skillsRequired;
        ...
        let newinterviewing = [];
        params.userInfo.appliedJobs.forEach((item,index) => {
          newinterviewing[index]=item.jobId;
        });
        console.log('newinterviewing',newinterviewing)
        const interviewings = await db.job.find({ _id: { $in: newinterviewing }, jobTitle: new RegExp(params.search,"i") }, { maxSalary: 1, companyId: 1, minSalary: 1, jobTitle: 1,  jobPostedDate: 1, skillsRequired: 1 }).populate({path:"companyId", select: "fullName companyLogo address "}).lean();
        ...
        let newoffered = [];
        params.userInfo.appliedJobs.forEach((item,index) => {
          newoffered[index]=item.jobId;
        });
        console.log('newoffered',newoffered)
        const offereds = await db.job.find({ _id: { $in: newoffered }, jobTitle: new RegExp(params.search,"i") }, { maxSalary: 1, companyId: 1, minSalary: 1, jobTitle: 1,  jobPostedDate: 1, skillsRequired: 1 }).populate({path:"companyId", select: "fullName companyLogo address "}).lean();
...
const jobFilters = async (params) => {
    ...
    if (params.showResultType == 1) {
        ...
        if (params.city && params.city != "") {
            obj["address.city"] = { $in: params.city }
        }
...
const jobFormat = async (data, check1) => {
    ...
    if (data.length != "") {
        for (var i = 0; i < data.length; i++) {
            ...
            if(data[i].address){
                data[i].companyFullAddress = data[i].address.fullAddress
                data[i].companyCity = data[i].address.city                 
            }
...
console.log({ status: params.status, companyId: params.userId, isDeleted: false  }, { _id: 1, status: 1, address: 1, selectcurrencyid: 1, jobPostedDate: 1, jobTitle: 1, startDate: 1, skillsRequired: 1, minSalary: 1, maxSalary: 1, endDate: 1 })
    ...
    //await data.forEach(async (obj) =>
    {
        // console.log('>>>>>>>>>>>>>>>>>>>',data1[counter].skillsRequired)
...
console.log(">>>>>>>>>>",data.document,data.document.pvrId,data.document.certificateId.length >0 && data.document.pvrId != '' && data.document.cvId != '' && data.document.cvId != null,data.document.certificateId.length >0 && data.document.pvrId != ''&& data.document.pvrId != null && data.document.cvId == '' || data.document.cvId == null);    
if(data.document.certificateId.length >0 && data.document.pvrId != '' && data.document.cvId != '' && data.document.cvId != null) {
    ...
    else if(data.document.certificateId.length >0 && data.document.pvrId != ''&& data.document.pvrId != null && data.document.cvId == '' || data.document.cvId == null) {
    ...
    else if(data.document.certificateId.length = 0 && data.document.pvrId == '' && data.document.cvId == '' && data.document.pvrId == undefined 
...
const fetchEmployeeReview = async (params) => {
	...
	const check1 = await db.users.findOne({ _id: params.empId }, { review: 1 })
    if (check1.review != "") {
        let counter = 0
        let res = []
        let data1 = {}
        ...
        for (let i = 0; i < check1.review.length; i++) {
            // const userData = await db.users.findOne({ _id: check1.review[i].from }, { lean: true })
			 const userData = await db.users.findOne({ _id: check1.review[i].from })
