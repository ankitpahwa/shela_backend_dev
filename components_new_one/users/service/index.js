...
const employerProfileStep1 = async (params) => {
    ...
    // add new params denoting the video status whether commercial or not
    try {
...
const employeeProfileStep4 = async (params) => {
    ...
    console.log('emplopyeeprofilesetp4',params)
...
const accountPrivacy = async (params) => {
    ...
    log.end()
    console.log('accountPrivacy working properly')
    return data
}
...
const followUnfollowCompany = async (params) => {
    ...
    log.end()
    console.log('followUnfollowCompany working properly')
    //return data
}
...
const followingJobs = async (params) => {
    ...
    const res = services.jobFormat(data, check1)
...
const saveJobs = async (params) => {
    ...
    console.log("saveJobs api working")
    log.end()
}
...
const viewSavedJobs = async (params) => {
    ...
    for (var i = 0; i < data.length; i++) {
        console.log('data[i].companyName[0].address',data[i].companyName[0].address)
        if(data[i].companyName[0].address != undefined){
...
const applyForJob = async (params) => {
    ...
    if (params.applyingWithType == 2) //with pvr
    {
        ...
        if (pvastatus == 1) {
            console.log('inside the if if condition')
...
const acceptRejectFromCandidate = async (params) => { 
    ...
    console.log("acceptRejectCandidate jobs api working")
    log.end()
}
...
const selectInterviewTime = async (params) => {
    ...
    console.log("select Interview Time for  jobs api working")
    log.end()
    // return data,data1
}
...
const viewReferredJobs = async (params) => {
    ...
    console.log("view Referred Jobs  api working")
    log.end()
    return data
...
//     // to check the saved job status
//     const check1 = await db.users.findOne({ _id: params.userId }, { savedJobs: 1 }, { lean: true })
//     const data = await db.users.findOne({ _id: params.userId, "appliedJobs.applicationStatus": { $in: [1, 2, 3, 4] } }, { appliedJobs: 1 }, { lean: true });
...
const viewAppliedJobs = async (params) => {
    ...
    // to check the saved job status
    const check1 = await db.users.findOne({ _id: params.userId }, { savedJobs: 1 }, { lean: true })
    const data = await db.users.find({ _id: params.userId},{ appliedJobs: 1 }, { lean: true });
    var array = []
    if (data == null){
        return []
    }
    ...
    data.forEach(value =>{
        value.appliedJobs.forEach(data =>{
            if(data.applicationStatus == 1 || data.applicationStatus == 2 || data.applicationStatus == 3 || data.applicationStatus == 4){
                array.push(data.jobId)
            }
        }
        )
    })
    ...
    //  { appliedJobs: 1 }, { lean: true });
     for (const iterator of userDetails.appliedJobs) {
         if (iterator.applicationStatus != 5) {
             let jobDetails = await db.job.findOne({ _id: iterator.jobId ,"appliedJobs.applicationStatus":3}, { jobTitle: 1, companyId: 1 }, { lean: true })
             .populate({
                 path: "companyId",
                 select: "fullName"
                })
...
const viewInterviewing = async (params) => {
    ...
    if (data) {
        ...
        for(let value of data){
            ...
            for(let data of value.appliedJobs){
                    // console.log('data',data)
                if(data.applicationStatus == 4){
                    if(data.jobId != null){
                    x  = await db.skills.find({ "_id": { $in: data.jobId.skillsRequired } }, { name: 1, _id: 1 }, { lean: true });
                    y = await db.users.findOne({ _id: data.jobId.companyId });
                    }
                    let obj1 = {
                        _id:data.jobId._id,
                        jobInviteStatus: data.jobInviteStatus,
                        jobTitle: data.jobId.jobTitle,
                        description: data.jobId.description,
                        maxSalary: data.jobId.maxSalary,
                        minSalary: data.jobId.minSalary,
                        skillsRequired: x,
                        jobPostedDate: data.jobId.jobPostedDate,
                        savingStatus: data.jobId.savingStatus,
                        disableEligibleStatus: data.jobId.disableEligibleStatus,
...
//         let counter = 0;
//         data.forEach(value => {
//         value.appliedJobs.forEach(async data => {
//             if(data.applicationStatus == 4){
//                 console.log('case1')
//                 if(data.jobId != null){
//                     x  = await db.skills.find({ "_id": { $in: data.jobId.skillsRequired } }, { name: 1, _id: 1 }, { lean: true });
...
const viewOfferedJobs = async (params) => {
    ...
    const check1 = await db.users.findOne({ _id: params.userId }, { savedJobs: 1 }, { lean: true });
    const data = await db.users.find({ _id: params.userId, "appliedJobs.applicationStatus": 5 }, { appliedJobs: 1 }, { lean: true })
    if (data == null){
        return []
    }
    var array = []
...
const candidateStats = async (params) => {
    ...
    {
        "$match": {"appliedCandidates.applicationStatus": 1}
    },
    { $count: "applicants" },
...
const viewInvitedJobs = async (params) => {
    ...
    console.log('dataaa',data.jobInvites)
    var res = []
    ...
    console.log("view Invited Jobs  api working")
    log.end()
    return res.reverse();
}
...
const searchFeaturedCandidate = async (params) => {
    ...
    //     const data2 = await db.users.find({
    //         profileStepCompleted: 4,
    //         employmentInfo: {
    //             $elemMatch: {
    //                 jobTitle: new RegExp(params.jobTitle, 'i')
    //             }
    //         }
    //     }, { _id: 1, fullName: 1, totalExperience: 1, employmentInfo: 1 })
    //     // let counter=0
    //     // for (let x = 0; x < data1.length; x++) {
    //     //     for (let y = 0; y < data1[x].employmentInfo.length; y++) {
    //     //         if (data1[x].employmentInfo[y].jobTitle == params.jobTitle) {
    //     //             data2[counter] = data1[x]._id
    //     //             counter++
    ...
    // if (params.pincode && params.pincode != 0) {
    //     const check2 = await db.users.find({ profileStepCompleted: 4, userRole: "employee" }, { _id: 1, "address.pincode": 1 })
    //     for (let x = 0; x < check2.length; x++) {
    //         if (check2[x].address.pincode == params.pincode) {
    //             data2[counter] = check2[x]._id
    //             counter++
    //         }
    //     }
    ...
    // const data3 = (value, index, self) => /*filter( function*/ {
    //     return self.indexOf(value) === index;
    // } //);
    // uniqueArray = data2.filter(data3)


    // // uniqueArray = [ ...new Set(data2) ]
    // // uniqueArray = data2.filter(function(elem, pos) {

    // //     return data2.indexOf(elem).toString() == pos.toString();
    // // })
    // // let data3=[]
    // // data3 = data2.filter( function( item, index, inputArray ) {
    ...
    // let data = []
    // if (data2 == "" && params.pincode != -1 && params.jobTitle != "") {

    //     data = await db.users.find(obj, { _id: 1, fullName: 1, totalExperience: 1, profilePicId: 1, isWorking: 1, "document.pvrId": 1 }, { lean: true })

    // } else if (data2 != "" && params.skills != "") {

    //     for (let j = 0; j < data2.length; j++) {
    //         data[j] = await db.users.find({ _id: data2[j], userRole: "employee", profileStepCompleted: 4, skills: { $in: params.skills } }, { _id: 1, fullName: 1, totalExperience: 1, profilePicId: 1, isWorking: 1, "document.pvrId": 1 })
    //     }

    // } else if (data2 != "" && params.skills == "") {

    //     for (let j = 0; j < data2.length; j++) {
    //         data[j] = await db.users.find({ _id: data2[j], profileStepCompleted: 4, userRole: "employee" }, { _id: 1, fullName: 1, totalExperience: 1, profilePicId: 1, isWorking: 1, "document.pvrId": 1 })
    //     }

    // } else if (params.jobTitle == "" && params.skills == "" && params.pincode == -1) {

    //     data = await db.users.find({ profileStepCompleted: 4, subscriptionStatus: 1, userRole: "employee" }, { _id: 1, fullName: 1, totalExperience: 1, profilePicId: 1, isWorking: 1, "document.pvrId": 1 })
...
const socialLoginSignUp = async (params) => {
    ...
    console.log('insdie the socialLoginSignUp')
    let dataToSave = {}
    ...
    console.log('params.facebookLoginId',params.facebookLoginId)
